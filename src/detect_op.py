from src.classes import Bkp, Junction

def detect_intra_inter_trans(srt_read_aligns, options, repeat_regions, non_local_thresh=10000):
    """
    ...
    for i in range(len(srt_read_aligns) - 1):
        ...
        # # same chrom
            ...
            # at least one type in subtype string
            if len(junc_sub_type) != 0:
                # append to list
                read_include_juncs.append(Junction(junc_type, "-".join(junc_sub_type), [cur_align, next_align], [bkp1, bkp2]))
